    ...
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <div className="rounded-xl border border-[var(--border)] bg-[var(--card)] p-10 text-center">
          ...
          <a
            href="/overview"
            className="inline-flex px-5 py-2.5 rounded-lg bg-[var(--primary)] text-white text-sm font-medium hover:opacity-90 transition-opacity"
          >
            Ver meu dashboard
          </a>
        </div>
          ...
          <div key={s.key} className="flex items-center gap-2">
            ...
            {idx < ONBOARDING_SECTIONS.length - 1 && (
              <div
                className={cn(
                  'flex-1 h-px w-8',
                  completed.has(idx) ? 'bg-green-500' : 'bg-[var(--border)]'
                )}
              />
            )}
          </div>
